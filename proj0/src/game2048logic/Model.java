...
public class Model {
    ...
    public boolean atLeastOneMoveExists() {
        if(emptySpaceExists()){
            ...
            for(int x = 0;x < board.size();x++){
                for(int y = 0;y < board.size();y++){
                    int n = tile(x,y).value();
                    // Check the element above
                    if(y < board.size()-1 && n==tile(x,y+1).value()) return true;
                    // Check the element below
                    if(y > 0 && n == tile(x,y-1).value())  return true;
                    // Check the element left
                    if( x > 0 && n == tile(x-1,y).value()) return true;
                    // Check the element right
                    if(x < board.size()-1 && n== tile(x+1,y).value()) return true;
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        if(targetY < board.size() -1){
            ...
            if(myValue == upperTile.value() && !upperTile.wasMerged()){
                board.move(x, targetY + 1, board.tile(x, targetY));
    ...
    public void tiltColumn(int x) {
        ...
        /*for(int i = 0;i < board.size();i++){
            for(int j = board.size()-1;j >= 0;j--){
                if(board.tile(i,j)!=null)  moveTileUpAsFarAsPossible(i,j);
            }
        }*/
        for(int j = board.size()-1;j >= 0;j--){
